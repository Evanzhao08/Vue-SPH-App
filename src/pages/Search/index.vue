            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                ...
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a href="#">价格<span v-show="isTwo" class="iconfont" :class="{'icon-long-arrow-up':isAsc,'icon-long-arrow-down':isDesc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
...
export default {
  ...
  //当组件挂载之前执行一次【先与mount之前】
  beforeMount() {
    //console.log(this.$route.query);
  },
  mounted() {
    // this.searchParams.category1Id = this.$route.query.category1Id
    // Object.assign:ES6新增的语法合并对象
    Object.assign(this.searchParams, this.$route.query, this.$route.params);
    this.getData();
  },
  computed: {
    ...mapGetters(["goodsList"]),
    ...mapState({total:(state)=>state.search.searchList.totalPages}),
    isOne(){
  ...
  methods: {
    ...
    attrInfo(attr,attrValue){
      ...
      if(this.searchParams.props.indexOf(props) == -1){
        this.searchParams.props = [...this.searchParams.props,props]
      }
       this.getData();
